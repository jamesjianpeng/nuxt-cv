<template>
    <div>
        ...
        <ul>
            <li>
                1. [vuex] duplicate getter key: getCount，请打开控制台查看
            </li>
            <li>
                2. 为什么 sate 和 mutations 不会出现 警告：（1） state 默认是有命名空间。（2）mutations 的每个方法都是存放在该方法的 name 下面的数组中，请打开控制台查看，或者查看下面的图👇
            </li>
            <li>
                3. 调用 $store.commit('increment') 这个方法，没有使用 namespaced 的 module 提交相同的 state 的话会同时改变，请点击 👉 <button @click="$store.commit('increment')">inscrement</button>
                <div>
